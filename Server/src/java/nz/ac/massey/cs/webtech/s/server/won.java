 ...
 */
package nz.ac.massey.cs.webtech.s_17138916.server;
...
public class won extends HttpServlet {
    ...
    public static String checkWin(String[][] gameState) {
        ...
        // If there no winner then set winner to none
        if (winner.equals("")) {
            winner = "none";
        }       
        return winner;
    ...
    private static String checkRows(String[][] gameState, String winner) {
        for (String[] gameState1 : gameState) {
            int crossCount = 0;
            int noughtCount = 0;
            for (String gameState11 : gameState1) {
                if (gameState11.equals("x")) {
                    crossCount += 1;
                } else if (gameState11.equals("o")) {
                    noughtCount += 1;
                }
            }
            if (crossCount == 3) {
    ...
    private static String checkDraw(String[][] gameState, String winner) {
        //If a empty space is found, return from method
        for (String[] gameState1 : gameState) {
            for (String gameState11 : gameState1) {
                if (gameState11.equals("_") || gameState11.equals("")) {
                    return winner;
                }
            }
        }
        //The game is in draw if no empty space has been found
        winner = "draw";
        return winner;
    }
}
